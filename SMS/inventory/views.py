...
def sales_order_list(request):
    ...
    # 5. SORTING (ALL RECORDS BEFORE PAGINATION)
    sort_by = request.GET.get('sort', '').strip()
    sort_order = request.GET.get('order', 'asc').strip()

    if sort_by == 'order_number':
        from django.db.models.functions import Cast, Substr
        orders_queryset = orders_queryset.annotate(
            order_num_int=Cast(
                Substr('order_number', 5),  # strips "ORD-" → gives number
                output_field=IntegerField()
            )
        ).order_by(
            'order_num_int' if sort_order == 'asc' else '-order_num_int'
        )
    elif sort_by == 'total_amount':
        orders_queryset = orders_queryset.order_by(
            'total_amount' if sort_order == 'asc' else '-total_amount'
        )
    else:
...
def add_sales_order(request):
    ...
    if request.method == "POST":
        ...
        consolidated_items = OrderedDict()
        ...
        for i in range(len(product_ids)):
            ...
            if p_id in consolidated_items:
                consolidated_items[p_id] += qty
            else:
                consolidated_items[p_id] = qty
        ...
        #  LOOP THROUGH CONSOLIDATED ITEMS
        
        for p_id, qty in consolidated_items.items():
        ...
        # Calculate per-company order count safely by looking at the last order
        last_so = SalesOrder.objects.filter(company=profile.company).order_by('-id').first()
        new_num = 1
        if last_so and last_so.order_number and last_so.order_number.startswith('ORD-'):
            try:
                new_num = int(last_so.order_number.split('-')[1]) + 1
            except ValueError:
                new_num = SalesOrder.objects.filter(company=profile.company).count() + 1
        elif last_so:
            new_num = SalesOrder.objects.filter(company=profile.company).count() + 1
        ...
        order = SalesOrder.objects.create(
            company=profile.company,
            order_number=f"ORD-{new_num:02d}",
...
def sales_order_detail(request, pk):
    ...
    for item in items: 
        ...
        item.formatted_price = format_currency(
            unit_price,
            'INR',
            locale='en_IN'
        )

        item.formatted_subtotal = format_currency(
            item.subtotal,
            'INR',
            locale='en_IN'
        )

    order.formatted_total = format_currency(
        order.total_amount,
        'INR',
        locale='en_IN'
    )
...
def purchase_order_list(request):
    ...
    # PO NUMBER SORTING (NUMERIC)
    if sort == 'po':

        orders_list = orders_list.annotate(
            po_number_int=Cast(
                Substr('order_number', 4),
                IntegerField()
            )
        )
        ...
        if direction == 'desc':
            ...
            orders_list = orders_list.order_by(
                '-po_number_int'
            )
...
# 16-4-26
    ...
    for order in orders:
        data.append([
            ...
            Paragraph(f"Rs. {format_inr(order.total_amount)}", cell_style),
...
def add_purchase_order(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            last_po = PurchaseOrder.objects.filter(company=profile.company).exclude(id=order.id).order_by('-id').first()
            new_num = 1
            if last_po and last_po.order_number and last_po.order_number.startswith('PO-'):
                try:
                    new_num = int(last_po.order_number.split('-')[1]) + 1
                except ValueError:
                    new_num = PurchaseOrder.objects.filter(company=profile.company).count()
            elif last_po:
                new_num = PurchaseOrder.objects.filter(company=profile.company).count()
                
            order.order_number = f"PO-{new_num:02d}"
...
def add_vendor(request):
    ...
    if request.method == "POST":
        ...
        if not email:
            ...
            errors["email"] = "Only .in or .com email addresses are allowed."
        elif "gnial" in email.lower():
            errors["email"] = "Typo detected: Did you mean @gmail.com?"
        elif Vendor.objects.filter(company=company, email=email).exists():
            errors["email"] = "Vendor with this email already exists."
...
def purchase_order_detail(request, pk):
    ...
    if request.method == "POST":
        ...
        for item_id in selected_items:
            ...
            qty_key = f"received_qty_{item_id}"
            received_qty = request.POST.get(qty_key)
            ...
            if product.is_expiry_tracked:
                ...
                batch_number = item.batch_number or "NO-BATCH"
                expiry_date = None
        ...
        messages.success(request, "Selected items received successfully")
...
def export_sales_pdf(request):
    ...
    data = [[
        ...
        Paragraph("Cost Price", header_cell_style),
        Paragraph("Amount (Rs)", header_cell_style),
    ]]
    ...
    for item in purchases:
        ...
        data.append([
            Paragraph(item.order.created_at.strftime("%d-%m-%Y"), cell_style),
            Paragraph(item.product.name, cell_style),
            Paragraph(str(quantity), cell_style),
            Paragraph(indian_currency(cost_price), cell_style),
            Paragraph(indian_currency(amount), cell_style),
        ])
    ...
    data.append([
        ...
        Paragraph(indian_currency(total_amount), cell_style)
    ])

    # -------------------------------
    # Table Design
    # -------------------------------
    table = Table(data, colWidths=[80, 220, 50, 90, 100])
...
def format_indian_currency(value):
    ...
    try:
        ...
        if len(integer_part) > 3:
            ...
            integer_part = ",".join(pairs) + "," + last_three
...
def purchase_order_print(request, pk):
    ...
    for item in order.items.all():
        ...
        items.append({
            ...
            "formatted_cost_price": format_indian_currency(item.cost_price or 0),
            "formatted_subtotal": format_indian_currency(item.subtotal or 0),
        })
        

    
    formatted_total = format_indian_currency(
        order.total_amount or 0
    )

    formatted_tax = format_indian_currency(0)
...
def export_sales_orders_pdf(request):
    ...
    for order in sales_orders:
        ...
        data.append([
            ...
            order.status,
            f"Rs. {format_inr_indian(order.total_amount)}",
...
def export_inventory_pdf(request):
    ...
    for p in products:
        data.append([
            ...
            Paragraph(
                f"Rs. {format_inr(p.selling_price)}",
                cell_style
            ),
        ])
